...
def query(
    ...
    **model_kwargs,
    ...
    # - o1, o3, etc. (reasoning models)
    # - gpt-5 series
    # - codex-mini-latest
    model = filtered_kwargs.get("model", "")
    if (
        re.match(r"^o\d", model)
        or model.startswith("gpt-5")
        or model == "codex-mini-latest"
    ):
        filtered_kwargs.pop("temperature", None)

    # Handle reasoning_effort for thinking models (gpt-5, o1, o3, etc.)
    ...
    if use_chat_api:
        ...
        tools_list = []
        if func_spec is not None:
            tools_list.append(func_spec.as_openai_responses_tool_dict)
            filtered_kwargs["tool_choice"] = func_spec.openai_responses_tool_choice_dict
        # Add web search if enabled
        if filtered_kwargs.pop("web_search", False):
