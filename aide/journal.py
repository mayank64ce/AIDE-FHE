...
class Node(DataClassJsonMixin):
    ...
    # ---- FHE-specific fields (optional, for backward compatibility) ----
    # Lookup evidence from OpenFHE API inspection
    lookup_evidence: object = field(default=None, kw_only=True)  # LookupEvidence or None

    # Validator JSON result (parsed from validator output)
    validator_json_result: dict | None = field(default=None, kw_only=True)

    # Testcase information
    testcase_type: str | None = field(default=None, kw_only=True)  # sanity, search, eval
    testcase_seed: int | None = field(default=None, kw_only=True)
    testcase_summary: str | None = field(default=None, kw_only=True)
...
class Journal(DataClassJsonMixin):
    ...
    def generate_summary(self, include_code: bool = False, include_buggy: bool = True) -> str:
        """Generate a summary of the journal for the agent.

        Args:
            include_code: Whether to include full code in summary
            include_buggy: Whether to include buggy attempts (helps avoid repeating mistakes)
        """
        summary = []

        # Good nodes with accuracy-based labels
        for n in self.good_nodes:
            # Determine label based on accuracy
            metric_val = n.metric.value if n.metric and n.metric.value is not None else 0
            if metric_val >= 0.7:
                label = "[GOOD]"
                hint = ""
            elif metric_val >= 0.4:
                label = "[MODERATE]"
                hint = " Consider improving the model."
            elif metric_val > 0:
                label = "[LOW ACCURACY]"
                hint = " Try a different approach."
            else:
                label = "[ZERO ACCURACY]"
                hint = " Need a completely different approach."

            summary_part = f"{label} Design: {n.plan}\n"
            if include_code:
                summary_part += f"Code: {n.code}\n"
            summary_part += f"Results: {n.analysis}{hint}\n"
            summary_part += f"Accuracy: {metric_val:.1%}\n"
            summary.append(summary_part)

        # Include recent buggy attempts to avoid repeating mistakes
        if include_buggy:
            # Only include leaf buggy nodes (most recent failures)
            recent_buggy = [n for n in self.buggy_nodes if n.is_leaf][-3:]  # Last 3
            for n in recent_buggy:
                # For failed attempts, truncate plan to essential info (first 300 chars)
                plan_brief = n.plan[:300] + "..." if n.plan and len(n.plan) > 300 else n.plan
                summary_part = f"[FAILED] Approach: {plan_brief}\n"
                summary_part += f"Error: {n.analysis}\n"
                summary.append(summary_part)
